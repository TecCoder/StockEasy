...
export function Dashboard({
  ...
  const current =
    portfolios.data?.find((portfolio) => portfolio.id === selected) ??
    portfolios.data?.[0];
    ...
    <>
      ...
      {portfolios.loading ? (
        ...
        <section className="panel">
          <Empty title="Tu historial empieza aquí">
            <p>
              Crea una cartera y registra tus operaciones para ver el resumen.
            </p>
            <Link to="/portfolio">Crear cartera →</Link>
          </Empty>
        </section>
      ) : (
        <>
          <div className="tabs">
            {portfolios.data.map((portfolio) => (
              <button
                key={portfolio.id}
                className={current?.id === portfolio.id ? "selected" : ""}
                ...
                <section className="panel">
                  <div className="panel-heading">
                    ...
                    <span className="badge">
                      {snapshot.data.positions.length} POSICIONES
                    </span>
                  </div>
                  {snapshot.data.positions.length ? (
                        ...
                        <tbody>
                          {snapshot.data.positions.map((position) => (
                            <tr key={position.asset_id}>
                              <td>
                                <Link to={`/asset/${position.asset_id}`}>
                    ...
                    <Empty title="Todavía no tienes posiciones">
                      <p>
                        Registra una compra o transferencia de entrada en esta
                        cartera.
                      </p>
                    </Empty>
                  )}
