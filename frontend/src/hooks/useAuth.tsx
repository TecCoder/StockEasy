...
type Auth = { user: User | null; loading: boolean; login: (username: string, password: string) => Promise<void>; shutdown: () => Promise<void> };
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function shutdown() { await post('/auth/logout-and-shutdown'); setCsrf(''); setUser(null); }
  return <AuthContext.Provider value={{ user, loading, login, shutdown }}>{children}</AuthContext.Provider>;
}
