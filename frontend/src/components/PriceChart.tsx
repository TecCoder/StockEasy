...
function chartValue(value: number, format: ChartValueFormat, unit: string): string {
  if (format === 'percent') {
    return `${new Intl.NumberFormat('es-ES', { maximumFractionDigits: 1 }).format(value * 100)} %`;
  }
  if (format === 'compact') {
    const absolute = Math.abs(value);
    const scale: [number, string] = absolute >= 1e12
      ? [1e12, 'T']
      : absolute >= 1e9
        ? [1e9, 'B']
        : absolute >= 1e6
          ...
          : absolute >= 1e3
            ? [1e3, 'mil']
            : [1, ''];
    const formatted = new Intl.NumberFormat('es-ES', { maximumFractionDigits: 2 }).format(value / scale[0]);
    return `${formatted}${scale[1] ? ` ${scale[1]}` : ''}${unit ? ` ${unit}` : ''}`;
  }
  const maximumFractionDigits = format === 'price' && Math.abs(value) < 0.01 ? 8 : 2;
  const formatted = new Intl.NumberFormat('es-ES', { maximumFractionDigits }).format(value);
  return `${formatted}${unit ? ` ${unit}` : ''}`;
}

function exactChartValue(value: number, format: ChartValueFormat, unit: string): string {
  const normalized = format === 'percent' ? value * 100 : value;
  const maximumFractionDigits = format === 'price' && Math.abs(value) < 0.01 ? 8 : format === 'percent' ? 4 : 4;
  const formatted = new Intl.NumberFormat('es-ES', { maximumFractionDigits }).format(normalized);
  return `${formatted}${format === 'percent' ? ' %' : unit ? ` ${unit}` : ''}`;
}

function displayedTime(time: Time | undefined): string {
  if (time == null) return '';
  if (typeof time === 'string') return new Intl.DateTimeFormat('es-ES').format(new Date(`${time}T00:00:00`));
  if (typeof time === 'number') return new Intl.DateTimeFormat('es-ES').format(new Date(time * 1000));
...
export function PriceChart({
  ...
  useEffect(() => {
    ...
    if (prices.every(p => p.open != null && p.high != null && p.low != null)) {
      ...
      tooltipSeries.push({ series: candles as ISeriesApi<SeriesType>, name: seriesLabel });
      ...
      tooltipSeries.push({ series: line as ISeriesApi<SeriesType>, name: seriesLabel });
    ...
    if (volumes.length) {
      ...
      volume.setData(volumes.map(p => ({ time: p.date as Time, value: Number(p.volume) })));
    }
    overlays.forEach(overlay => {
      ...
      tooltipSeries.push({ series: series as ISeriesApi<SeriesType>, name: overlay.name });
      series.setData(overlay.points.map(point => ({ time: point.time as Time, value: point.value })));
    });
    chart.subscribeCrosshairMove(param => {
      if (!tooltip.current || !container.current || !param.point || !param.time) {
      ...
      const values = tooltipSeries.flatMap(item => {
        ...
        return value == null ? [] : [`${item.name}: ${exactChartValue(value, valueFormat, unit)}`];
      });
      if (!values.length) {
      ...
      const left = param.point.x > container.current.clientWidth - tooltipWidth
  ...
  return <div className="chart-shell" aria-label={`${seriesLabel}. Eje Y: ${axisLabel}. Eje X: fecha.`}><div className="chart-axis-legend"><span><strong>Eje Y</strong> · {axisLabel}{scaleHint}</span><span><strong>Eje X</strong> · Fecha</span></div><div className="chart-plot"><div ref={container} style={{ height, width: '100%' }} /><div ref={tooltip} className="chart-tooltip" hidden /></div></div>;
}
