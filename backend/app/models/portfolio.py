...
class Transaction(Base):
    ...
    input_currency: Mapped[str | None] = mapped_column(String(3))
    input_price: Mapped[Decimal | None] = mapped_column(ExactDecimal)
    input_fees: Mapped[Decimal | None] = mapped_column(ExactDecimal)
    input_taxes: Mapped[Decimal | None] = mapped_column(ExactDecimal)
    input_to_asset_rate: Mapped[Decimal | None] = mapped_column(ExactDecimal)
    conversion_provider: Mapped[str | None] = mapped_column(String(64))
    conversion_effective_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    conversion_precision: Mapped[str | None] = mapped_column(String(16))
